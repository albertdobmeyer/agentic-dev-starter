...
8. **Bootstrap self-audit** (zero-trust verification; do NOT skip): Before proceeding to planning, verify each item. Block on any failure and complete it before continuing.
   ...
   - **claude-mem (recommended, non-blocking)**: Run `npx claude-mem status 2>/dev/null` to check if persistent session memory is active. If not installed, remind the human: `npx claude-mem install` once per machine. This is NOT a gate blocker — the workflow runs without it — but teams skipping it lose cross-session context continuity at handoff boundaries.
