...
- **Protocol A step 3** documents adapter auto-detection plus clean fallback to ask-the-human for mixed teams.
- **Protocol A step 1** adds SKIP branch to YES/PARTIAL/NONE tree.
...
- **`npx tiged` delivery path**. 1-command install, around 260 KB payload, no git history:
...
- **Tone scrub**: removed service-company branding ("AKD AUTOMATION SOLUTIONS") from user-facing docs. LICENSE copyright attribution kept (legitimate).

### Fixed

- **`--integration cursor` to `cursor-agent`** (4 locations: CURSOR.md step 4, CURSOR.md prose, adapters/cursor/README.md role-mapping table, misc references).
